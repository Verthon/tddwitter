...
type ExtractTags<T extends string> = T extends `${string}<${infer Tag}>${infer Rest}`
  ? Tag extends `/${string}` 
    ? ExtractTags<Rest>
...
export type MessageValues<T extends string> = 
  (ExtractVariables<T> extends never ? {} : Record<ExtractVariables<T>, string | number>) &
  (ExtractTags<T> extends never ? {} : Record<ExtractTags<T>, TagToComponent<ExtractTags<T>>>);
